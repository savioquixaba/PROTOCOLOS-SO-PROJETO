...
def sendata(sock,nome,host,port):
	...
	while True:
		...
		if msg_to_send[0:10] == "/DOWNLOAD ":
			...
			if dados.decode("ANSI") =="Arquivo não existe!!":
				soc.close()
			else:	
...
def recvdata(sock,vet):
	sock.bind((socket.gethostbyname(socket.gethostname()),9000))
	while True:
		os.system("clear||cls")
		for men in vetor:
			print(men)
		msg, recpt = sock.recvfrom(1024)
		if recpt:
			print(f"{msg.decode()}") 
...
b = threading.Thread(target=recvdata, args=(s_rcv,vetor,))
b.start()
